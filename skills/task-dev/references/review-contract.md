...
| # | 規則 |
|---|---|
| IM-1 | **`files` は必須キー**。`C-9` / `C-11` のコミット対象集合は `files ∪ 修正委譲の files ∪ dev-result.md` の和集合として成立する（実装を委譲する経路＝複数タスクモードと `--team` 指定時の単一タスクモード。委譲しない単一タスクモードでは `changed_by_skill` が同じ役割を担う）。`git add -A` の禁止（`C-1`）を維持するには、この集合が戻り値から得られることが必須条件である |
