...
上記の URL 有無の分岐を判定した **後**、各実行手順に着手する前に進捗管理の方式を確定します（この判定は後述の URL 取得手段の探索とは別物です）。登録するのは、分岐判定で確定した系列のフェーズのみです。
...
登録するのは URL 有無の分岐で **確定した系列のみ** です（URL 未指定時と指定時でフェーズ集合が
異なるため、判定後に登録します）。
