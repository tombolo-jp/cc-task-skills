...
上記の URL 有無の分岐を判定した **後**、各実行手順に着手する前に、進捗管理用の Task ツールを
ロードしてください（deferred tool のため、ロードしないと呼び出せません）。この ToolSearch は
進捗管理のためのものであり、後述の URL 取得手段の探索とは別物です。
...
**Task 方式（主経路）**: 確定した系列の全フェーズを `TaskCreate` で1件ずつ登録し、各フェーズの
着手時に `TaskUpdate` で `in_progress` へ、完了時に `completed` へ遷移させます。`subject` は
フェーズ名（imperative 形）、`description` にはそのフェーズで行うことを1文で記述します。

**チェックリスト方式（フォールバック）**: 確定した系列の全フェーズを `- [ ]` 形式のチェックリスト
として応答中に宣言し、各フェーズの完了直後にチェックリストを再掲して `- [x]` へ更新します。
**フォールバックしたことを告知するメッセージは表示しないでください**（チェックリスト自体が
進捗表示を兼ねます）。

いずれの方式でも登録（宣言）は **省略不可** です（成果物の必須要素）。登録するのは URL 有無の
分岐で **確定した系列のみ** です（URL 未指定時と指定時でフェーズ集合が異なるため、判定後に
登録します）。列挙順に実行し、どのフェーズもスキップ・前倒しをしないこと。

**URL が指定されていない場合（4フェーズ・この順序）:**

1. プロジェクトルートを pwd で解決する
2. タスクディレクトリを作成する
3. init.md（空テンプレート）を作成する
4. req.md（空テンプレート）を作成する

**URL が指定されている場合（7フェーズ・この順序）:**

1. プロジェクトルートを pwd で解決する
2. 取得方式を決定する（MCP 優先 → ブラウザ系フォールバック）
3. URL 本文・コメントを取得する
4. 添付ファイルを取得する
5. タスクディレクトリを作成し init.md を書き込む
6. 部分取得の未取得項目を init.md に明記する
7. req.md（空テンプレート）を作成する

> **取得手段ゼロ時の扱い（URL 指定時のみ）**: フェーズ2「取得方式を決定する」で利用可能な取得手段が
> 一つも見つからない場合は、フェーズ2 を `completed` にせず **`in_progress` のまま残し**、後続フェーズ
> （フェーズ3〜7）も **`pending` のまま残して** エラー終了してください（フォールバック時は該当項目を
> 未完 `- [ ]` のまま残します）。`deleted` は「不要になった」を意味し失敗を表さないため使用しません。
> これは既存仕様「取得手段ゼロ = ディレクトリ・init.md・req.md を一切作らずエラー終了」と整合します
> （未達フェーズが未完状態として可視化されます）。
>
...
本文・コメントに添付ファイルがある場合は、`<PROJECT_ROOT>/.claude/tasks/$ARGUMENTS[0]/files/` を作成して保存してください。
...
1. Bash ツールでディレクトリを作成（添付がある場合は files も）:
   ```
   mkdir -p <PROJECT_ROOT>/.claude/tasks/$ARGUMENTS[0]
   ```
...
# 添付ファイル
- files/<filename>（元: <attachment URL>）
<添付が無ければ「添付なし」と記載>
