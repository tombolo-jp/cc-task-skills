...
`- [ ]` 形式のチェックリストとして応答中に宣言してください（URL 未指定時と指定時でフェーズ集合が
異なるため、判定後に宣言します）。この宣言は省略不可です（成果物の必須要素）。
宣言後は列挙順に実行し、どのフェーズもスキップ・前倒しをしないこと。各フェーズの完了直後に
チェックリストを再掲し、完了項目を `- [x]` へ更新して進捗を明示します。
...
> 一つも見つからない場合は、フェーズ2 を `- [x]` にせず未完（`- [ ]`）のまま中断し、後続フェーズ
> （フェーズ3〜7）も未完のまま残してエラー終了してください。これは既存仕様「取得手段ゼロ =
> ディレクトリ・init.md・req.md を一切作らずエラー終了」と整合します（未達フェーズが未完チェックで
> 可視化されます）。
>
