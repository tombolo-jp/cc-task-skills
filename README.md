...
/task-verify your-task-name --team   # Agent Teams モード（正常系検証設計・周辺検証／自動化推奨・削減レビューを並行実施）
```
- **機能**: 開発成果物に対する簡潔な手動検証手順書を生成
- **用途**: 開発・修正完了後の手動検証
- **入力ファイル**: req.md, design.md, dev-result.md（+ review.md, fix-result.md が存在する場合）
- **作成ファイル**: `.claude/tasks/{your-task-name}/verify.md`

検証手順書 `verify.md` が作成されます。各項目は `- [ ] <操作> → <期待結果>` の1行形式で、コピペ可能なコマンドや具体的な期待結果が含まれます（期待結果が操作から自明な場合は省略されます）。所要時間は文書冒頭に合計として1回だけ記載され、簡単なQuick Winから始まる構成で検証を始めやすくしています。自動テストで担保すべき確認観点は手順として展開せず、末尾の「付録: 自動化推奨の確認観点」セクションへ集約されます。
...
/task-verify my-task --team   # 正常系検証設計・周辺検証／自動化推奨・削減レビューを並行実施
```
