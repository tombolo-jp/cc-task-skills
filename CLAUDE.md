...
## 設計原則: フローは構造化チェックリストで明示的に追跡する

本プロジェクトの各スキルは「手順」セクションを地の文の Markdown で記述しているが、順守させたいワークフローは、手順の説明文に紛れさせず、**実行前に宣言する構造化チェックリスト**として明示的に追跡することを設計原則とする。

地の文の手順記述のみでは、フェーズのスキップ・実装の早期着手・フェーズの取りこぼしが起こりうる。これに対し、スキルの全フェーズを着手前に `- [ ]` 形式のチェックリストとして応答中へ宣言し、各フェーズの完了ごとに一覧を再掲して `- [x]` へ更新することで、進捗と未達を常に可視化する。

- 各スキルは「手順」冒頭（ステップ0）で自スキルの全フェーズをチェックリストとして宣言し、完了ごとに再掲・更新する。これがこの原則の具体化である。宣言は省略不可（成果物の必須要素）とし、スキップ・前倒し・完了マーク省略を禁止事項として明記する。
- これは「完璧な強制」ではなく「安定性の有意な向上」を狙う現実的スタンスである。ツールによる物理的強制に比べ強制力は下がり、順守は最終的に LLM に依存する。それでも地の文だけの手順記述に比べ、スキップ・前倒し・取りこぼしを構造的に起こりにくくする。
- 進捗追跡の記述にツール固有名を持ち込まない（役割語「チェックリスト」「フェーズ追跡」で記述する）。将来ハーネス側にタスク管理ツールが登場しても本文改修なしで共存でき、ツール名の変化で再び壊れることがない。
- `--team`（Agent Teams）成立時も、チームリードが自身のチェックリストで全フェーズを追跡する。チームメイトへ委任した作業は `Agent` の戻り値で完了を確認してからチェックを付ける。
- 今後スキルを追加・変更する際も、多段フローを持つスキルにはこの原則を適用すること。

> **経緯（失効済み）**: 2026-05-27 〜 2026-07-22 の間、本原則は「フローはツールで物理的に強制する」であり、全フェーズを組み込みのタスク登録／更新ツールで Task として登録し `in_progress` / `completed` 遷移させる方式を主経路、地の文チェックリストを例外経路としていた。その後 Claude Code 側の仕様変更により当該ツールが提供されなくなり（v2.1.217 実測: deferred tools 一覧に不在・ToolSearch でも取得不可）、主経路が恒久的に閉塞したため、例外経路であったチェックリスト方式を正式な主経路へ昇格させた。この経緯は現行の設計判断の根拠ではない。
...
> **フロー強制（全スキル共通）**: 9スキルすべてが「手順」冒頭の **ステップ0** で自スキルの全フェーズを `- [ ]` チェックリストとして宣言し、完了ごとに一覧を再掲して `- [x]` へ更新することで、スキップ・前倒し・取りこぼしを構造的に防ぐ（「[設計原則: フローは構造化チェックリストで明示的に追跡する](#設計原則-フローは構造化チェックリストで明示的に追跡する)」の具体化）。宣言は省略不可（成果物の必須要素）。**task-dev** は加えて、実行時に todo.md の各実装タスクをチェックリスト項目として動的追加し、1件ずつ着手・完了を明示する。**task-init** は URL 有無の分岐判定後に確定する系列のフェーズのみを宣言する特例とする。
...
`--team` を付けるだけで並列実行（Agent Teams）が起動します。環境変数の事前設定は **不要** です。スキルは `Agent` ツールでチームメイトを直接 spawn し、各チームメイトの成果を `Agent` の**戻り値**（最終メッセージが tool result として返る）で収集、進捗はチームリードのチェックリストで追跡します。
...
> **env 変数の役割（F-7）**: `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS` が現在ゲートしているのは `SendMessage`（teammate 間ライブ通信）ただ 1 つです。`Agent` による spawn と、チームリードのチェックリストによる進捗追跡は **この変数なしでも動作します**。したがって変数は「Agent Teams を動かすためのスイッチ」ではなく「SendMessage ライブ協調レイヤを有効化する任意スイッチ」です。
...
`--team` 指定時、スキルはライブ通信に用いるツールを以下でロードします。`Agent` は core ツールでメイン会話に常在するため、ToolSearch でのロードは **不要** です。進捗追跡はチームリードのチェックリストで行うため、ツールのロードを必要としません。
...
| 結果 | 扱い |
|------|------|
| `SendMessage` が `<functions>` ブロックに含まれる | ライブ通信を併用可。チームリードのチェックリストによる進捗追跡と合わせて利用する |
...
| `flow-checklist` | 廃止済みのタスク登録／更新ツール名が SKILL.md・CLAUDE.md（更新履歴節を除く）・README.md・テンプレートへ再混入していないか、および全9スキルにチェックリスト宣言を伴う `### ステップ0` が存在するか |
